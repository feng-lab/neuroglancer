export const glsl_emitCone = `
  void emitCone(float bradius, float tradius, vec4 attr_origin, vec4 attr_axis ) {
    ...
    vInvSqrHeight = 1.0 / vInvSqrHeight;
...
vec4 coneFragmentFunc(float bradius, float tradius, float invSqrHeight, float height) {
  vec3 rayOrigin = mix(vec3(0.0), vPoint, uOrtho);
  ...
  if (tradius - bradius < 0.001)  {   // cylinder
    ...
    vec3 P = diff * basis;
    vec3 factor = vec3(1.0, 1.0, - (tradius-bradius) * (tradius-bradius) * invSqrHeight);
    a0 = dot(P * P, factor);
    a1 = dot(P * D, factor);
    a2 = dot(D * D, factor);
  }
  ...
  float ratio = dot(ipoint-vBase, vec3(vTop-vBase)) * invSqrHeight;
